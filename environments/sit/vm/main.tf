...
module "test_vm" {
  ...
  root_iops      = "3000"
  # 填写安全组的name的名称
  security_groups = [local.secGroups["Sit_private_internal_linux_SecGroup"]]
  # 填写subnet_name名称
  subnet_id = local.subnets["sit_dmz_public_1a"]
...
module "test_windows_vm" {
  ...
  root_disk_type  = "gp3"
  root_iops       = "3000"
  security_groups = [local.secGroups["Sit_private_internal_windows_SecGroup"]]
  subnet_id       = local.subnets["sit_dmz_public_1a"]
  #   iam_instance_profile = "MySessionManagerRole"
  description = "Compute Ubuntu Server"
